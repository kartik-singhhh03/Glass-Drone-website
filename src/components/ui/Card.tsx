...
const Card = ({ children, className = '', hover = false, border = true, padding = 'md' }: CardProps) => {
  ...
  return (
    <div className={`${baseStyles} ${borderStyles} ${hoverStyles} ${paddings[padding]} ${className}`}>
      {children}
    </div>
  );
};

...
const desktopNodes = [
  {
    ...
    y: "50%",
    textAlign: "text-left",
    posOpts: "top-[80px] left-0 w-[280px]",
  },
  {
    x: "27.5%",
    y: "25%",
    textAlign: "text-center",
    posOpts: "top-[80px] left-1/2 -translate-x-1/2 w-[280px]",
  },
  {
    x: "72.5%",
    y: "75%",
    textAlign: "text-center",
    posOpts: "bottom-[80px] left-1/2 -translate-x-1/2 w-[280px]",
  },
  {
    x: "95%",
    y: "50%",
    textAlign: "text-right",
    posOpts: "top-[80px] right-0 w-[280px]",
  },
];
...
const HowItWorks = () => {
  ...
  return (
    <section
      id="how-it-works"
      ref={sectionRef}
      className="bg-white relative pt-[60px] md:pt-[80px] pb-16 md:pb-24 overflow-hidden z-10 w-full"
    >
      {/* --- FOREGROUND CONTENT --- */}
      <div className="max-w-[1240px] mx-auto px-6 w-full">
        ...
        <div className="desktop-container relative w-full max-w-[1000px] h-[500px] mx-auto hidden lg:block select-none pointer-events-none mt-4">
          ...
          >
            ...
            <path
              id="flight-path"
              d="M 50,200 Q 275,0 500,200 T 950,200"
              fill="none"
              stroke="#E2E8F0"
              strokeWidth="4"
          ...
          <div
            id="drone-wrapper"
            className="absolute top-0 left-0 w-[90px] h-[90px] z-30 opacity-0 center origin-center"
          >
            ...
            <div className="drone-anim-layer w-full h-full relative group">
              <img
                loading="lazy"
                src="/d-2.webp"
                alt="Drone Path Tracker"
                className="w-full h-full object-contain relative z-10 scale-110 drop-shadow-[0_8px_16px_rgba(0,0,0,0.15)]"
              />
            </div>
          </div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div
                key={i}
                className="absolute z-20 pointer-events-auto step"
              ...
              >
                ...
                <div className="relative -translate-x-1/2 -translate-y-1/2">
                  ...
                  <div
                    className={`w-[46px] h-[46px] rounded-full flex items-center justify-center font-extrabold text-[18px] border-[4px] shadow-lg transition-colors duration-[600ms]
                ...
                <div
                  className={`step-text absolute ${node.posOpts} ${node.textAlign} transition-all duration-700 pointer-events-none 
                    ${isPassed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                >
                  <h3 className="text-[22px] font-bold text-slate-900 mb-3 leading-tight tracking-tight shadow-white drop-shadow-md">
                    {t(`how.items.${step.key}.title`)}
                  </h3>
                  <p className="text-[15px] text-slate-600 leading-[1.7] font-medium">
                    {t(`how.items.${step.key}.desc`)}
                  </p>
                </div>
              </div>
            );
        ...
        <div className="mobile-container relative w-full lg:hidden flex flex-col gap-[50px] pl-[50px] py-10 mx-auto max-w-[500px]">
          ...
          <div className="absolute top-10 bottom-16 left-[28px] w-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              className="mobile-active-line w-full bg-blue-accent relative rounded-full"
              style={{ height: 0 }}
            >
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-[60px] h-[60px] z-30 pointer-events-none">
                <div className="drone-anim-layer w-full h-full relative">
                  <img
                    ...
                    src="/d-3.webp"
                    alt="Drone Scrubber"
                    className="w-full h-full object-contain rotate-90 drop-shadow-[0_8px_16px_rgba(0,0,0,0.15)]"
                  />
                </div>
              </div>
            </div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div
                key={`mob-${i}`}
                className="relative w-full z-20 flex flex-col pl-6 pr-4"
              >
                {/* Left Side Node */}
                <div className="absolute -left-[44px] top-6 -translate-y-1/2">
                  ...
                  <div
                    className={`w-[44px] h-[44px] rounded-full flex items-center justify-center font-bold text-[18px] border-[4px] shadow-lg transition-colors duration-[600ms]
                ...
                {/* Content Block */}
                <div
                  className={`transition-all duration-700 bg-white/50 py-2
                    ${isPassed ? "opacity-100 translate-y-0" : "opacity-30 translate-y-4"}`}
                >
                  <h3 className="text-[22px] font-bold text-slate-900 mb-2 leading-[1.3] tracking-tight">
                    {t(`how.items.${step.key}.title`)}
                  </h3>
                  <p className="text-[15px] sm:text-[16px] text-slate-500 leading-[1.7] max-w-[320px] font-medium">
                    {t(`how.items.${step.key}.desc`)}
                  </p>
                </div>
              </div>
